# TODO: Fonbet & Pari & BetBoom Collector v2.9.3
...
### F-6.6: Расчёт остатка баланса для каждой операции

**Статус:** выполнено v2.9.3

Добавлено поле `balance` (остаток на счёте) для каждой операции в экспорте:

- [x] Реализован метод `_fetchCurrentBalance()` — получение текущего баланса из `/session/info`
- [x] Реализован метод `_calculateBalances()` — расчёт баланса для всех операций
- [x] Добавлено поле `balance` в `operations[]` массив для всех категорий (bets, fastBets, freebets, finance, bonus)
- [x] Добавлено поле `currentBalance` в `account` секцию экспорта
- [x] Обновлён `exportOperations()` — async метод с получением баланса
- [x] Обновлён `GitHubSync.sync()` — получение баланса перед формированием данных

**Детали:** Поле "Остаток" отображается в UI страницы Fonbet, но не приходит из API. Страница вычисляет его на клиентской стороне по формуле: `balance_before = balance_after - sum`. Формула универсальна для всех типов операций, так как API возвращает `sum` с правильным знаком (положительная для выигрышей/депозитов, отрицательная для ставок/выводов).
...
**Статус:** выполнено v2.9.1

Обнаружены и добавлены 3 новых типа операций:

- [x] Operation ID 8: BET_SOLD_CANCELLED (Отмена продажи) - 1 случай
- [x] Operation ID 41: TAX (Налог) - 8 случаев, всегда идёт в паре с 89
- [x] Operation ID 89: WITHDRAWAL_NET (Вывод) - 10 случаев, сумма вывода после налога
- [x] Обновлены OPERATION_TYPES, OPERATION_GROUPS, OPERATION_NAMES
- [x] Документация в CONTEXT.md (таблицы + lifecycle примеры)

**Детали:** Операции 89 и 41 всегда появляются вместе в одном marker — 89 это чистая сумма вывода, 41 это налог. Последовательность: 460 (холдирование) → 461 (отмена холдирования) → 89 (вывод) → 41 (налог).
...
<details>
<summary>Фазы 1–26 (v1.15.x → v2.9.0) — нажмите чтобы развернуть</summary>
